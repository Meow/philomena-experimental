defmodule PhilomenaWeb.Admin.User.ForceFilterController do
  ...
  def new(conn, %{"user_id" => slug}) do
    with {:ok, user} <- Users.load_user_for_force_filter(conn.assigns.current_user, slug) do
  ...
  def create(conn, %{"user_id" => slug, "user" => user_params}) do
    with {:ok, user} <- Users.admin_force_filter(conn.assigns.current_user, slug, user_params) do
      conn
  ...
  def delete(conn, %{"user_id" => slug}) do
    with {:ok, user} <- Users.admin_unforce_filter(conn.assigns.current_user, slug) do
      conn
