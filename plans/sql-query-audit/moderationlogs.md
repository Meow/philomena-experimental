...
- The `id DESC` addition is a deterministic-pagination correctness improvement for equal `created_at` values, not evidence by itself that a new index is worthwhile. Run `EXPLAIN (FORMAT JSON)` for the actual Scrivener page/count query on a representative moderation-log table before considering `(created_at DESC, id DESC)`; compare against the existing `(created_at)` index and account for insert/cleanup write cost.
