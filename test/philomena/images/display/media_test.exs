defmodule Philomena.Images.Display.MediaTest do
  ...
  describe "render/2" do
    test "returns documented file and thumbnail shapes for a public image" do
      ...
      assert %Media{
               view_paths: {:file_paths, view_paths},
               download_paths: {:file_paths, download_paths},
               supplemental_paths: :none,
      ...
      assert thumbnails[:thumb] == %{
               path: exact(image, :thumb),
               width: 250,
               height: 125
             }

      assert thumbnails[:medium] == %{
               path: exact(image, :medium),
               width: 800,
               height: 400
             }

      assert thumbnails[:large] == %{
               path: "#{root()}/view/2024/1/2/42.png",
               width: 1000,
               height: 500
             }
    ...
    test "calculates constrained dimensions from a portrait aspect ratio" do
      ...
      assert thumb == %{path: exact(image, :thumb), width: 125, height: 250}
    end
    ...
    test "omits every media path before thumbnails are generated" do
      ...
      assert result.view_paths == :not_rendered
      assert result.download_paths == :not_rendered
      assert result.supplemental_paths == :not_rendered
    ...
    test "redacts every media path for a hidden image unless explicitly revealed" do
      ...
      assert result.view_paths == :not_available
      assert result.download_paths == :not_available
      assert result.supplemental_paths == :not_available
      assert result.thumbnails == :not_available
      refute inspect(result) =~ image.hidden_image_key

      revealed = Media.render(image, true)

      assert {:file_paths, %{short: short}} = revealed.view_paths
      assert short == exact(image, :full)
      assert short =~ image.hidden_image_key
      assert {:thumbnails, %{thumb: %{path: thumb_path}}} = revealed.thumbnails
    ...
    test "omits every media path for destroyed content" do
      ...
      assert result.view_paths == :destroyed
      assert result.download_paths == :destroyed
      assert result.supplemental_paths == :destroyed
  ...
  describe "format-specific supplemental paths" do
    test "provides a rendered PNG path for SVG images" do
      ...
      assert {:svg, %{static_preview: rendered, svg: svg}} = result.supplemental_paths
      assert rendered == "#{root()}/view/2024/1/2/42.png"
      assert svg == "#{root()}/view/2024/1/2/42.svg"
      assert {:file_paths, %{short: view, long: view_long}} = result.view_paths
      assert view == "#{root()}/view/2024/1/2/42.png"
      assert view_long == "#{root()}/view/2024/1/2/42__safe_artistfoo+bar_zebra.png"
      assert {:file_paths, %{short: download}} = result.download_paths
      assert download == "#{root()}/download/2024/1/2/42.svg"

      assert {:thumbnails, %{full: %{path: full_path}}} = result.thumbnails
      assert full_path == "#{root()}/view/2024/1/2/42.png"
      assert {:thumbnails, %{thumb: %{path: thumb_path}}} = result.thumbnails
    ...
    test "provides rendered, WebM, and MP4 paths for GIF images" do
      ...
      assert {:gif, paths} = Media.render(image, false).supplemental_paths
    ...
    test "provides rendered, MP4, and GIF preview paths for WebM images" do
      ...
      assert {:webm, %{static_preview: rendered, mp4: mp4, gif_previews: previews}} =
               Media.render(image, false).supplemental_paths
      ...
      assert previews[:thumb] == %{path: exact(image, :thumb, "gif"), width: 250, height: 125}
    end
  end
end
